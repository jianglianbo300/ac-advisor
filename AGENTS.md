...
- [ ] **B 类：`load_state()` 失败静默返回 default 偏危险（2026-08-14 审查发现）**：BOM 事件证明"状态文件损坏 → 假装没状态继续跑"会悄悄丢失 MIN_OFF 锚点。应至少打印 `[ERROR] state load failed: ...`；更严格做法是**状态文件损坏时自动控制进入 fail-safe（本次 tick 不执行开/关）**。下一轮排期
