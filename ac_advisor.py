...
升级内容：
- v11.0: RC 热模型、DP 最优调度、自学习闭环、TTS 语音
- 保留：露点判据、热质量学习、自适应阈值、季节自适应、室内传感器、天气获取
"""
import json
import math
import os
...
def should_precool(wx, current_hi, threshold_hi):
    ...
    if not times or not temps: return False, None, None
    now_h = datetime.now().hour
    idx = None
    for i, t in enumerate(times):
        if len(t) >= 13 and t[11:13] == f"{now_h:02d}":
            idx = i; break
    if idx is None: return False, None, None
...
def evaluate_and_learn(state, now_ts):
    ...
    for entry in log:
        ...
        if action in ("cooling", "dehumid"):
            ...
            elif temp_drop < 0.3 and hum_drop < 3: success = False
        elif action in ("off", "fan"):
            temp_rise = cur_temp - pre_temp
            if temp_rise > 2.0 or (cur_hum is not None and cur_hum > 80): success = False
...
def log_decision(state, action, pre_temp, pre_hum, now_ts):
    ...
    log.append({
        "time": now_ts, "action": action,
        "pre_temp": pre_temp, "pre_hum": pre_hum,
        "evaluated": False, "power_at_decision": power_at_decision,
    })
    learned["decision_log"] = log[-50:]
    save_learned(learned)
...
def record_thermal_event(event_type, temp_before, temp_after, duration_min, outdoor_temp):
    ...
    events.append({
        "type": event_type, "temp_before": temp_before,
        "temp_after": temp_after, "duration_min": duration_min,
        "outdoor_temp": outdoor_temp,
        "timestamp": datetime.now().isoformat()
    })
    data["events"] = events[-100:]
    # 每 5 个新事件或 24 小时重拟合一次
    ...
    if not _should_fit and _last_fit:
        try:
            ...
            if _age > 86400: _should_fit = True
...
def read_indoor(timeout=3.0):
    ...
    if temp is not None and hum is not None: return temp, hum
...
def apply_state_from_verify(state, new_mode, real, now_ts):
    ...
    if real == "on":
        if new_mode in ("cooling", "dehumid", "dehumid_alert"):
            if not was_on:
                state["run_start"] = now_ts; state["last_on_at"] = now_ts
